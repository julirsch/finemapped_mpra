...
to_plot_out <- data.frame(matrix(ncol = 11, nrow = 0))
names(to_plot_out)<- c("attribute","cell_type","tissue_type","precision","recall","total","pos_for_attr","prec_upper","prec_lower","rec_upper","rec_lower")
prlist = c('cell_type_specific_CRE','cell_type_specific_CRE_and_mpra')
...
eqtl_out <- lapply(annotlist, function(x) {prec_rec_ct(x, prset_df)}) %>%
  bind_rows() %>%
...
names(clrs) = c("HCT116", "HEPG2", "K562","SKNSH","any")


p1 <- ggplot(data = eqtl_out %>% dplyr::filter(cohort == 'eQTL',
  ...
  #geom_text(aes(label = round(precision, digits = 3)), nudge_x = 0.001, nudge_y=0.001, size=5)+
  ggtitle('eQTLs')

p1

# Save off part of supplementary table 10
eqtl_out %>% dplyr::filter(cohort == 'eQTL',
                           annot %in% c("is.e2g","e2g_yes_emVar_yes",
                                        "is.CRE", "CRE_yes_emVar_yes")) %>%
  vroom::vroom_write(paste0('tables/s9g.txt'))
...
cowplot::save_plot(paste0('figures.s9g.pdf'),
...
cowplot::save_plot(paste0('figures/s9g.legend'),
                   leg,
                   base_height = 6,
                   base_width = 6,
                   device = cairo_pdf
