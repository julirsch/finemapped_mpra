...
load("/mnt/sdb/gtex_mpra/code/motifs_processed_20230225.RData")
...
#variant_chip_lite_df <- vroom::vroom("/mnt/sdb/gtex_mpra/data/variant_chip_lite.txt.gz")
#variant_chip_lite_df <- vroom::vroom("/mnt/sdb/gwas_eqtl_mpra/data/annotations/motif/variant_chip_lite.txt.gz")
#motifdisrupt_chip_df <- vroom::vroom("/mnt/sdb/gtex_mpra/data/motifdisrupt_chip_jaspar_hocmoco_only.txt.gz")
...
# Compute correlations between motif disruption and allelic effect
motif_corr_df <- mpra_motif_df %>%
  dplyr::filter(motif_call_nodiff == T, !is.na(antigen), emVar_any == T) %>%
  distinct(variant, cell_type, motif, TF, log2Skew, motif_diff) %>%
  group_by(motif, TF, cell_type) %>%
  dplyr::filter(length(motif) > 10) %>%
  dplyr::mutate(log2Skew = rank(log2Skew),
                motif_diff = rank(motif_diff)) %>% 
  dplyr::summarize(n = length(log2Skew),
                   cor = cor(log2Skew, motif_diff, method = "pearson"),
                   lowci = stats::cor.test(log2Skew, motif_diff)$conf.int[1],
                   highci = stats::cor.test(log2Skew, motif_diff)$conf.int[2],
                   pvalue = stats::cor.test(log2Skew, motif_diff)$p.value) %>%
  ungroup() %>% 
  dplyr::mutate(fdr = qvalue::qvalue(pvalue)$qvalue)

# Write out TF motif correlations (Supplemental Table 13)
motif_corr_df %>%
  vroom::vroom_write("tables/stable13.txt.gz")

# Collapse across same TF by picking the most significant
motif_corr_df <- motif_corr_df %>%
  group_by(TF, cell_type) %>%
  dplyr::filter(pvalue == min(pvalue)) %>%
  dplyr::filter(row_number() == 1) %>%
  ungroup() %>% 
  dplyr::mutate(sig = ifelse(fdr < 0.05, T, F)) %>% 
  dplyr::group_by(TF) %>% 
  dplyr::filter(fdr == min(fdr)) %>% 
  dplyr::filter(row_number() == 1) %>%
  ungroup()

# Number of significant TFs correlated with MPRA skew
motif_corr_df %>%
  dplyr::count(sig)

# Filter to only significant TF motifs
motif_corr_sig_df <- motif_corr_df

# Examples for text
motif_corr_df %>%
  dplyr::filter(sig == T) %>%
  arrange(-cor) %>%
  dplyr::select(TF, cell_type, n, cor, fdr) %>%
  dplyr::filter(grepl("GATA|TP5|HNF4|SNAI|GFI|ETS1|ELF4|CEBPG|CTCF|BCL6", TF))

# Plot allelic effect correlations with motif disruption effects (Fig. 2d)
p1 <- motif_corr_df %>%
  dplyr::filter(sig == T) %>%
  ggplot(aes(y = cor, x = reorder(TF, -cor), color = cell_type)) +
  geom_linerange(aes(ymin = lowci, ymax = highci), linewidth = 0.05) +
...
cowplot::save_plot(
  paste0("figures/fig2/fig2d_motifcorr.pdf"),
  plt_combined,
  base_height = 2,
  base_width = 2.2,
  device = cairo_pdf
...
motif_plot <- bind_rows(m_enrich(mech_motif_df %>% dplyr::mutate(annot = ifelse(variant %in% tmp_motif_call$variant, 1, 0)), "annot", "Breaks motif"),
                        ...
                        m_enrich(mech_motif_df %>% dplyr::mutate(annot = ifelse(enf_chip + enf_dhs > 0, 1, 0)), "annot", "Enformer"),
                        m_enrich(mech_motif_df %>% dplyr::mutate(annot = ifelse((enf_chip + enf_dhs > 0) & (variant %ni% tmp_motif_call$variant), 1, 0)), "annot", "Enformer no motif"))
...
mech_motif_df %>%
  ...
  vroom::vroom_write("/mnt/sdb/gwas_eqtl_mpra/tables/stable12.txt.gz")
